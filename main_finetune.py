...
def main(config):
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        train_one_epoch(config, model, criterion, data_loader_train, optimizer, epoch, mixup_fn, lr_scheduler, scaler, writer)
        if dist.get_rank() == 0 and (epoch % config.SAVE_FREQ == 0 or epoch == (config.TRAIN.EPOCHS - 1)):
            save_checkpoint(config, epoch, model_without_ddp, max_accuracy, optimizer, lr_scheduler, scaler, logger)

        acc1, acc5, loss = validate(config, data_loader_val, model)
        writer.add_scalar('acc1', acc1, global_step=epoch)
        logger.info(f"Accuracy of the network on the {len(dataset_val)} test images: {acc1:.1f}%")
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, scaler, writer):
    ...
    for idx, (samples, targets) in enumerate(data_loader):
        ...
        if mixup_fn is not None:
            samples, s_targets = mixup_fn(samples, targets)
        ...
        with mcontext():
            with amp.autocast(enabled=True):
                ...
                loss = criterion(outputs, s_targets)
        ...
        loss_meter.update(loss.item(), s_targets.size(0))
