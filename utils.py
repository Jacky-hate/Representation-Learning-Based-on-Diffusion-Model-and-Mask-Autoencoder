...
def load_pretrained_sample(config, model, logger):
    logger.info(f"==============> Loading weight {config.PRETRAINED} for sampling......")
...
def load_pretrained(config, model, logger):
    ...
    if config.MODEL.TYPE == 'vit':
        ...
        logger.info(f">>>>>>>>>> Remapping pre-trained keys for VIT_LDM ..........")
        checkpoint = remap_pretrained_keys_vit(model, checkpoint_model, logger)
    else:
