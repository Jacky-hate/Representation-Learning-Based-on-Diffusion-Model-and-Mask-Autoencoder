...
def sample(config, data_loader, model):
    ...
    path = os.path.join(config.OUTPUT, f'val_one_ep500_1024')
    ...
    for idx, (images, masks, _) in enumerate(data_loader):
        images = images.cuda(non_blocking=True).repeat(128,1,1,1)
        masks = masks.cuda(non_blocking=True).repeat(128,1,1)
