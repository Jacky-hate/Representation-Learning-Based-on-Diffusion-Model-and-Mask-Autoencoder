...
def parse_option():
    ...
    parser.add_argument('--no-sp-checkpoint', action='store_true',
                        help="whether to use gradient checkpointing to save memory")
...
@torch.no_grad()
def sample(config, model):
    model.eval()

    batch_time = AverageMeter()
    
    
    path = os.path.join(config.OUTPUT, f'uncond')
    ...
    for idx in range(10):
        shape = (config.DATA.BATCH_SIZE, 3, config.DATA.IMG_SIZE, config.DATA.IMG_SIZE)
        # compute output
        denoised_imgs, denoised_imgs_process = model.sample_uncon(shape)
        ...
        #logger.info(f'psnr {psnr/config.DATA.BATCH_SIZE} ssim {ssim/config.DATA.BATCH_SIZE}')
